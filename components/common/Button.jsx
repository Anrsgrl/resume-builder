...
const Button = ({ children, animation, icon, ...props }) => {
  return (
    <button
      {...props}
      type="button"
      className="w-fit flex items-center justify-center gap-1 animation-all group overflow-hidden w-fit px-3 py-1 rounded-lg text-main cursor-pointer border border-main hover:border-main hover:bg-main group font-semibold"
    >
      {animation && (
